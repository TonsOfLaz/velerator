...
global $fullpath, $startdirectory, $project_array;
...
function createProject ($projectname, $projectfile, $extra) {
	...
	shell_exec("cp $startdirectory/velerator_files/views/all.blade.php $fullpath/resources/views/pages/all.blade.php");
	shell_exec("cp $startdirectory/velerator_files/views/single.blade.php $fullpath/resources/views/pages/single.blade.php");
	...
	// ===================================> WELCOME PAGE
	$appview = file_get_contents($startdirectory."/velerator_files/views/app.blade.php");
	$newappview = str_replace("[APPNAME]", ucwords($projectname), $appview);
	file_put_contents($fullpath."/resources/views/app.blade.php", $newappview);
	...
	if (file_exists("$project_files/welcome.blade.php")) {
		...
		$welcome = file_get_contents($startdirectory."/velerator_files/views/welcome.blade.php");
	}
	
	file_put_contents($fullpath."/resources/views/welcome.blade.php", $welcome);
	$jsfile_links = "";
	// ===================================> CREATE PAGES
	...
	";
				...
				if (strpos("a".$filecore, "sections/") > 0) {
					...
					$generic_page = file_get_contents($startdirectory."/velerator_files/views/page.blade.php");
					$newpage = str_replace("[TITLE]", ucwords($filecore), $generic_page);
					$newpage = str_replace("[CONTENT]", $file_str, $newpage);
				}
				
				file_put_contents($fullpath."/resources/views/".$filename, $newpage);
				break;
		}
	}
	$app_template = file_get_contents($fullpath."/resources/views/app.blade.php");
	$new_app_template = str_replace("[JSFILES]", $jsfile_links, $app_template);
	file_put_contents($fullpath."/resources/views/app.blade.php", $new_app_template);
	...
	*/
	...
	if (isset($project_array['LISTFUNCTIONS'])) {
		foreach ($project_array['LISTFUNCTIONS'] as $object => $function_name_titles) {
			...
			$oldview = file_get_contents($fullpath."/resources/views/pages/$object.blade.php");
			$newview = str_replace("[LISTFUNCTIONS]", $str, $oldview);
			file_put_contents($fullpath."/resources/views/pages/$object.blade.php", $newview);
	...
	$welcome = file_get_contents($fullpath."/resources/views/welcome.blade.php");
...
function createNewPageView($viewname, $singular_objects) {
	...
	$baseview = file_get_contents("./resources/views/pages/all.blade.php");
	$oldstr = "[TITLE]";
	$newstr = $uppercase;
	...
	$baseview = file_get_contents("./resources/views/pages/single.blade.php");
	$oldstr = "[OBJECT]";
	$newstr = '$'.$lowercase;
	$newview = str_replace($oldstr, $newstr, $baseview);
	file_put_contents("./resources/views/pages/".$lowercase.".blade.php", $newview);
}
...
function addFunctionToModel($model_name, $function_name, $function_text) {
